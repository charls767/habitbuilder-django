...
class AdminScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('AdministraciÃ³n')),
      body: access.when(
        data: (isAdmin) => isAdmin ? const _AdminTabs() : const _Forbidden(),
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (error, _) => _AdminError(error: error, onRetry: () => ref.invalidate(adminAccessProvider)),
      ),
    );
  }
}
    ...
    child: Column(
      children: [
        const TabBar(tabs: [
          ...
          Tab(icon: Icon(Icons.flag_outlined), text: 'ModeraciÃ³n'),
        ]),
        const Expanded(child: TabBarView(children: [_UsageTab(), _UsersTab(), _ModerationTab()])),
      ],
    ),
  );
}
...
class _UsageTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppContent(child: usage.when(
      data: (value) => RefreshIndicator(
        onRefresh: () async => ref.refresh(adminUsageProvider.future),
        child: ListView(padding: const EdgeInsets.all(20), children: [
          Text('Resumen de uso', style: Theme.of(context).textTheme.headlineSmall),
          const SizedBox(height: 8),
          Text('${_date(value.from)} - ${_date(value.to)}', style: Theme.of(context).textTheme.bodySmall),
          const SizedBox(height: 16),
          Wrap(spacing: 12, runSpacing: 12, children: [
            _Metric(label: 'Usuarios activos', value: value.activeUsers),
            _Metric(label: 'Registrados', value: value.registeredUsers),
            _Metric(label: 'HÃ¡bitos creados', value: value.habits),
      ...
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (error, _) => _AdminError(error: error, onRetry: () => ref.invalidate(adminUsageProvider)),
    ));
  }
}
...
class _UsersTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppContent(child: users.when(
      ...
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (error, _) => _AdminError(error: error, onRetry: () => ref.invalidate(adminUsersProvider)),
    ));
  }
}
...
class _UserCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) => Card(child: ListTile(
    ...
    trailing: Row(mainAxisSize: MainAxisSize.min, children: [
      IconButton(
        tooltip: user.status == 'activo' ? 'Suspender usuario' : 'Reactivar usuario',
        icon: Icon(user.status == 'activo' ? Icons.pause_circle_outline : Icons.play_circle_outline),
  ...
  Future<void> _changeStatus(BuildContext context, WidgetRef ref) async {
    ...
    final reason = await _reasonDialog(context, next == 'suspendido' ? 'Suspender usuario' : 'Reactivar usuario');
    if (reason == null || !context.mounted) return;
    try {
      await ref.read(adminRepositoryProvider).changeUserStatus(user.id, next, reason);
  ...
  Future<void> _changeRole(BuildContext context, WidgetRef ref, String role) async {
    if (role == user.role) return;
    final reason = await _reasonDialog(context, 'Cambiar rol a $role');
    if (reason == null || !context.mounted) return;
    try {
      await ref.read(adminRepositoryProvider).changeUserRole(user.id, role, reason);
      ref.invalidate(adminUsersProvider);
...
class _ModerationTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppContent(child: reports.when(
      data: (items) => RefreshIndicator(
        onRefresh: () async => ref.refresh(moderationQueueProvider.future),
        child: items.isEmpty
            ? ListView(children: const [SizedBox(height: 80), Center(child: Text('No hay reportes pendientes.'))])
            : ListView.separated(
                padding: const EdgeInsets.all(20), itemCount: items.length,
                separatorBuilder: (_, _) => const SizedBox(height: 10),
      ...
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (error, _) => _AdminError(error: error, onRetry: () => ref.invalidate(moderationQueueProvider)),
    ));
  }
}
...
class _ReportCard extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) => Card(child: ListTile(
    title: Text('PublicaciÃ³n ${report.publicationId}'),
    ...
    trailing: PopupMenuButton<String>(
      ...
      onSelected: (resolution) async {
        ...
        if (reason == null || !context.mounted) return;
        try {
          await ref.read(adminRepositoryProvider).resolveModeration(report.id, resolution, reason);
          ref.invalidate(moderationQueueProvider);
          if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reporte resuelto.')));
        } on ApiException catch (error) {
          if (context.mounted) _showError(context, error.message);
        }
      },
      itemBuilder: (_) => const [
        PopupMenuItem(value: 'ocultar', child: Text('Ocultar publicaciÃ³n')),
...
class _Metric extends StatelessWidget {
  ...
  final int value;
  @override
  Widget build(BuildContext context) => SizedBox(width: 170, child: Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('$value', style: Theme.of(context).textTheme.headlineMedium?.copyWith(color: AppColors.primary, fontWeight: FontWeight.w800)), const SizedBox(height: 4), Text(label)]))));
}

class _Forbidden extends StatelessWidget {
  const _Forbidden();
  @override
  Widget build(BuildContext context) => const Center(child: Text('No tienes permisos para acceder a esta secciÃ³n.'));
}
